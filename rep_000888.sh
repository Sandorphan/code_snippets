...
#  Name         :  All Port-ins
#  Description  :  Snapshot of all port-ins for yesterday
...
# PT    | 09/12/2004| 2.1     | Initial version
#********************************************************************#
   ...
   ****/
   ...
   SELECT a.*,
    ...
    WHERE a.memo_subscriber = s.subscriber_no
      ...
      AND (anl.expiration_date IS NULL OR anl.expiration_date > TRUNC(TO_DATE('$DATE', 'YYYYMMDD')))
      AND ba.ban = s.customer_id
      AND be.ban = s.customer_id
      AND be.ben = 1;

   CREATE TABLE port_ins2
   AS
   SELECT a.*,
          b.service_level
     FROM port_ins1 a,
   ...
   SELECT a.memo_ban||'|'||
    ...
    WHERE b.subscriber_no = a.memo_subscriber 
      ...
      AND (b.expiration_date IS NULL OR b.expiration_date > TRUNC(TO_DATE('$DATE', 'YYYYMMDD')))
      AND d.soc = b.soc
      AND(d.expiration_date IS NULL OR d.expiration_date > TRUNC(TO_DATE('$DATE', 'YYYYMMDD')))
      AND f.soc = b.soc
      AND f.tier_level_code IN (0, 1)
      AND(f.expiration_date IS NULL OR f.expiration_date > TRUNC(TO_DATE('$DATE', 'YYYYMMDD')));

   spool off;

   DROP TABLE port_ins1;
   DROP TABLE port_ins2;
